...
export default function CertificateClaim({
  ...
  const handleNext = () => {
    if (step === 'confirm') {
      ...
      if (celular.trim().length !== 9) {
        setError('El número de celular debe tener exactamente 9 dígitos válidos');
        return;
      }
      setStep('signature');
      setError(null);
  ...
  const generateAndUpload = async () => {
    if (!topic?.id) {
      setError('No se puede generar el certificado: curso no identificado.');
      return;
    }
    setStep('generating');
    setError(null);
    ...
    try {
      ...
      const result = await saveCertificate({
        ...
        cargo,
        celular,
        courseTitle: topic?.title || undefined,
        topicId: topic?.id || undefined,
        nota: (() => {
          const topicProgress = topic ? progress.filter(p => p.topicId === topic.id) : progress;
          const quizScores = topicProgress.filter(p => p.quizScore !== undefined).map(p => p.quizScore!);
  ...
  const shareViaWhatsApp = () => {
    if (!certUrl) return;
    const message = `Hola, aquí tienes mi certificado oficial de capacitación "${topic?.title || 'Capacitación'}": ${certUrl}`;
    ...
    <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center p-4 safe-area-top safe-area-bottom overflow-y-auto">
      ...
      <div className="w-full max-w-xl relative">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-slate-500 text-xs font-bold tracking-widest">VALIDACIÓN OFICIAL</p>
          </div>
            ...
            >
              ...
              <div className="space-y-4 mb-8">
                <div className="relative group">
                  <label className="text-[10px] text-blue-400 font-bold uppercase tracking-widest pl-1 mb-1 block">Nombre Completo</label>
                  <input
                    type="text"
                    value={nombre}
                    onChange={(e) => setNombre(e.target.value.toUpperCase())}
                    className="w-full px-4 py-3 rounded-xl bg-white/5 border border-white/10 text-white placeholder-slate-600 text-sm font-bold focus:border-blue-500/50 transition-all outline-none uppercase"
                  />
                </div>
          ...
          {step === 'generating' && (
            ...
            >
              ...
              <p className="text-slate-400 text-sm max-w-xs mx-auto leading-relaxed">
                Estamos ensamblando tu certificado oficial de "${topic?.title || 'Capacitación'}" y sincronizándolo con los servidores de Google...
              </p>
            </motion.div>
          )}
      ...
      <div className="fixed left-[-9999px] top-0 overflow-hidden" aria-hidden="true">
        <CertificateTemplate
          ...
          timestamp={timestamp}
          progress={topic ? progress.filter(p => p.topicId === topic.id) : progress}
          quizQuestions={topic ? quizQuestions.filter(q => q.idMain === topic.id) : quizQuestions}
          topic={topic}
          fullNameOverride={nombre}
